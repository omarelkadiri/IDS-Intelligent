...
class ZeekToNSLKDD:
    ...
    def monitor_real_time_logs(self, interval=60):
        """
        Surveille les logs Zeek en temps réel et les traite à intervalle régulier.

        Args:
            interval (int): Intervalle de temps entre chaque traitement (en secondes)
        """
        import time
        import sys
        from datetime import datetime
        ...
        try:
            while True:
                ...
                if self.connections:
                    ...
                    # Stocker dans ElasticSearch si l'option est activée
                    if self.es_integration and nslkdd_records:
                        self.store_in_elasticsearch(nslkdd_records)
                        print(f"Données envoyées à ElasticSearch: {len(nslkdd_records)} enregistrements")

                    # Écrire dans le fichier CSV (en mode append)
                    if nslkdd_records:
                        self.append_to_nslkdd_file(nslkdd_records)
                        print(f"Données ajoutées au fichier CSV: {len(nslkdd_records)} enregistrements")

                    # Réinitialiser le dictionnaire des connexions pour le prochain cycle
                    self.connections = {}
                else:
                    print("Aucune nouvelle connexion détectée.")
